...
angular.module('webrtcTestApp')
        .controller('PlayingCtrl', function ($rootScope, $scope, $log, $audioService, $synthService, $replayerService, $fxService) {
            $scope.globalScore = 0;
            //keep state of keys hold/down
            $scope.keysStatus = [false, false, false, false, false];
            $scope.userKeyHit = [false, false, false, false, false];
            $scope.userKeyHitInterval = undefined;
            $scope.instrument1 = 'instrument1';
            $scope.instrument2 = 'instrument2';
            $scope.instrument3 = 'instrument3';
            $rootScope.pMBSettings = {singleNoteScore: 10, heldNoteScore : 1};
            ...
            function doKeyUp(event) {
                ...
                if (note >= 0 && note <= 4) {
                    $scope.userKeyHit[note] = false;
                    if ($scope.userKeyHitInterval)
                    {
                    clearInterval($scope.userKeyHitInterval);
                    }
            ...
            $rootScope.$on('playmyband.user.noteDown', function (event, note, accumulatedNoteDelta, playerId) {
                ...
                //calculate score async to prevent canvas to be interrupted
                setTimeout(evaluateUserNote(note, accumulatedNoteDelta, playerId), 1);
            ...
            function scoreHeldNote(note){
                if ($scope.userKeyHit[note])
                {
                    $scope.globalScore = $scope.globalScore + $rootScope.pMBSettings.heldNoteScore;
                }
            }

            function evaluateUserNote( note, accumulatedNoteDelta, playerId)
            {
                ...
                {
                    $scope.userKeyHit[note] = true;
                    $scope.globalScore = $scope.globalScore + $rootScope.pMBSettings.singleNoteScore;
                    $scope.$digest();
                    $scope.userKeyHitInterval = setInterval(scoreHeldNote(note), $rootScope.pMBnoteErrorMarginMS);
                } else {
